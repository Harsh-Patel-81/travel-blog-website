...
const Auth = () => {
  ...
  const sendRequest = async (type = "login") => {
    const res = await axios.post(`http://localhost:2000/api/users/${type}`, {
      name: inputs.name,
      email: inputs.email,
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} >
        <Box style={{ backgroundColor: "white" }} maxWidth={400} display="flex" flexDirection={"column"} alignItems="center" justifyContent={"center"} boxShadow="10px 10px 20px #d84315" padding={3} margin='auto' marginTop={5} borderRadius={5}>
